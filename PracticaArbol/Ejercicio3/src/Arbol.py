...
class Arbol:
    ...
    def __buscar(self, valor, subArbol):
        if valor == subArbol.getDato() or subArbol == None:
            return subArbol
    ...
    def sucesores(self, nodo):
        self.__inOrden(nodo.getIzquierdo())
        self.__inOrden(nodo.getDerecho())
    
    def getPadre(self, subArbol, valor):
        if subArbol == None:
            print('no existe')
            return subArbol
        elif subArbol.getIzquierdo() != None and valor == subArbol.getIzquierdo().getDato():
            return subArbol
        elif subArbol.getDerecho() != None and valor == subArbol.getDerecho().getDato():
            return subArbol
        elif valor < subArbol.getDato():
            return self.getPadre(subArbol.getIzquierdo(), valor)
        elif valor > subArbol.getDato():
            return self.getPadre(subArbol.getDerecho(), valor)

    def getHermano(self, subArbol, valor):
        padre = self.getPadre(subArbol, valor)
        if padre.getIzquierdo() != None:
            if padre.getIzquierdo().getDato() == valor:
                return padre.getDerecho()
        if padre.getDerecho() != None:
            if padre.getDerecho().getDato() == valor:
                return padre.getIzquierdo()

    def nroNodos(self, nodo):
        if nodo == None:
            return 0 
        return 1 + self.nroNodos(nodo.getDerecho()) + self.nroNodos(nodo.getIzquierdo())
